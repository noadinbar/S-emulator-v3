...
setlocal
cd /d "%~dp0"
if not exist "ui-serverclient.jar" echo Missing ui-serverclient.jar & goto :fail
...
"%JAVA_EXE%" -Dfile.encoding=UTF-8 -Djava.library.path="%CD%\lib\jfx\bin" --module-path "%CD%\lib\jfx" --add-modules %JFX_MODULES% -jar "ui-serverclient.jar" %*
set "RC=%ERRORLEVEL%"
endlocal & exit /b %RC%
:fail
exit /b 1
